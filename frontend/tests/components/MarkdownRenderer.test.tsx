...
describe('preprocessMentions', () => {
  test('should parse user email mentions into markdown links', () => {
    ...
    expect(output).toBe('Tolong bantu [@user@example.com](mention-user://user@example.com) di sini.')
  })
  ...
  test('should parse multiple mentions in a single text', () => {
    ...
    const output = preprocessMentions(input)
    expect(output).toBe('Cc [@wardi](mention-user://wardi) silakan cek [#15](mention-card://15) dan [#16](mention-card://16)')
  })
})
